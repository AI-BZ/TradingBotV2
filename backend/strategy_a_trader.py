"""
Strategy A Trader - Conservative High-Confidence Trading
Real-time trading with moderate entry conditions
...
class StrategyATrader:
    """Strategy A: Conservative High-Confidence Live Trading

    Goals:
    - Moderate trade frequency (~809/day per symbol)
    - Good profit per trade (target $20-40)
    - High win rate (target >70%)

    Key Features:
    - Moderate entry: Standard volatility thresholds
    - Cooldown period: 5 minutes between entries
    - Wide BB range (0.45-0.55)
    """
    ...
    def __init__(
        ...
        logger.info(f"✅ StrategyATrader initialized")
        logger.info(f"   Symbols: {', '.join(symbols)}")
        logger.info(f"   Balance: ${initial_balance:,.2f}")
        logger.info(f"   Leverage: {leverage}x")
        logger.info(f"   Cooldown: {cooldown_seconds}s")
        logger.info(f"   Strategy: CONSERVATIVE HIGH-CONFIDENCE")

    async def start(self):
        """Start live trading"""
        logger.info("\n" + "="*80)
        logger.info("🚀 STARTING STRATEGY A TRADING")
        logger.info("="*80)
        logger.info(f"Strategy: Conservative High-Confidence")
        logger.info(f"Expected: ~809 trades/day per symbol")
        logger.info(f"Goal: Win Rate >70%, Avg Profit >$20/trade")
        logger.info("="*80 + "\n")
    ...
    async def _generate_and_execute_signals(self, symbol: str, tick: Tick):
        """Generate and execute CONSERVATIVE trading signals (Strategy A)"""
        ...
        # Generate CONSERVATIVE signal (Strategy A)
        signal = self._get_conservative_signal(symbol, indicators, tick.price)
    ...
    def _get_conservative_signal(self, symbol: str, indicators: dict, current_price: float) -> dict:
        """Strategy A: CONSERVATIVE signal generation

        Entry Conditions:
        - hybrid_volatility >= 0.04% (vs 0.08% in Strategy B)
        - atr_volatility >= 0.15% (vs 0.30% in Strategy B)
        - BB position: 0.45-0.55 (vs 0.48-0.52 in Strategy B)
        ...
        # STRATEGY A: CONSERVATIVE ENTRY CONDITIONS
                ...
                if 0.45 < bb_position < 0.55:
                    ...
                    if current_time - last_time >= self.cooldown_seconds:
                        ...
                        return {
                            'action': 'BOTH',
                            'confidence': 0.85,
                            'reason': f'CONSERVATIVE: H:{hybrid_pct:.3f}% A:{atr_pct:.3f}% BB:{bb_position:.3f}',
    ...
    async def get_performance(self) -> dict:
        ...
        return {
            ...
            'strategy': 'Strategy A - Conservative',
    ...
    async def stop(self):
        ...
        logger.info("\n" + "="*80)
        logger.info("🛑 STOPPING STRATEGY A TRADING")
        logger.info("="*80)
